...
class Agent:
    ...
    def train(self, max_episode, lr_rate, gamma, t_max, entropy_beta, path, names):
        workers = []
        for i in range(self.n_workers):
            worker_name = 'worker%i' % i
            workers.append(Worker(worker_name, self.env_name, self.global_actor, self.global_critic, max_episode, lr_rate, gamma, t_max, entropy_beta, path, names, self.global_episode_count, self.global_step))
...
class Worker(mp.Process):
    ...
    def __init__(self, name, env_name, global_actor, global_critic, max_episode, lr_rate, gamma, t_max, entropy_beta, path, names, global_episode_count, global_step):
        ...
        self.worker_actor  = Worker_Actor(self.state_dim, self.action_dim, self.action_bound, lr_rate[0], self.entropy_beta, self.global_actor) 
        self.worker_critic = Worker_Critic(self.state_dim, lr_rate[1], self.global_critic) 
        ...
        while self.global_episode_count.value <= int(self.max_episode):
            ...
            while not done:
                ...
                if len(batch_state) == self.t_max or done:
                    ...
                    v_values     = self.worker_critic.get_value(states)
                    next_v_value = self.worker_critic.get_value(next_state)
                    n_step_td_targets = self.n_step_td_target(rewards, next_v_value, done)
                    advantages = n_step_td_targets - v_values

                    self.worker_critic.update(states, n_step_td_targets)
                    self.worker_actor.update(states, actions, advantages)
